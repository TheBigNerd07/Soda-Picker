...
class DatabaseTests(unittest.TestCase):
    ...
    def test_manual_entry_can_be_logged_updated_and_deleted(self) -> None:
        ...
        entries = self.database.get_today_entries(self.local_now)
        ...
        updated = self.database.get_entry(entry.id)
        self.assertIsNotNone(updated)
        self.assertEqual(updated.soda_name, "Cold Brew Large")
        self.assertEqual(updated.caffeine_mg, 110)

        self.assertTrue(self.database.delete_entry(entry.id))
        self.assertEqual(self.database.get_today_entries(self.local_now), [])
    ...
    def test_soda_state_and_setting_overrides_persist(self) -> None:
        ...
        states = self.database.get_soda_state_map()
        ...
        overrides = self.database.get_setting_overrides()
    ...
    def test_recommendation_history_tracks_logged_state(self) -> None:
        ...
        history = self.database.get_recent_recommendations(limit=5)
    ...
    def test_passport_entries_can_be_logged_updated_exported_and_deleted(self) -> None:
        ...
        entries = self.database.list_passport_entries(limit=10)
        self.assertEqual(len(entries), 2)
        self.assertEqual(entries[0].soda_name, "Ramune")
        self.assertEqual(entries[1].country, "Peru")

        summary = self.database.get_passport_summary()
        ...
        updated = self.database.get_passport_entry(ramune.id)
        self.assertIsNotNone(updated)
        ...
        exported = self.database.export_passport_csv()
        self.assertIn("soda_name,brand,country", exported)
        self.assertIn("Ramune,Hata,Japan", exported)

        self.assertTrue(self.database.delete_passport_entry(ramune.id))
        remaining = self.database.list_passport_entries(limit=10)
    ...
    def test_wishlist_entries_can_be_added_updated_exported_and_deleted(self) -> None:
        ...
        entries = self.database.list_wishlist_entries(limit=10)
        self.assertEqual(len(entries), 2)
        self.assertEqual(entries[0].id, first_id)
        self.assertEqual(entries[0].status, "active")
        self.assertEqual(entries[1].id, second_id)

        existing = self.database.find_active_wishlist_entry(soda_name="Green River", brand="Sprecher")
        self.assertIsNotNone(existing)
        assert existing is not None
        self.assertEqual(existing.priority, 5)

        summary = self.database.get_wishlist_summary()
        self.assertEqual(summary.total_entries, 2)
        ...
        updated = self.database.get_wishlist_entry(first_id)
        self.assertIsNotNone(updated)
        assert updated is not None
        self.assertEqual(updated.status, "found")
        self.assertEqual(updated.priority, 4)

        exported = self.database.export_wishlist_csv()
        self.assertIn("soda_name,brand,country", exported)
        self.assertIn("Green River,Sprecher,United States", exported)

        self.assertTrue(self.database.delete_wishlist_entry(second_id))
        remaining = self.database.list_wishlist_entries(limit=10)
        self.assertEqual(len(remaining), 1)
